...
class CollaborateHome extends Component {
  ...
  render() {
    return (
      <div className="container nonEditorContainer" style={{marginTop: 10}}>
        <Link style={{textDecoration: 'none'}} to="/">
          <p className="logo">Capsule</p>
        </Link>
        {
          this.state.page === 'patchNotes' ? (
            <div className="dialogButton" style={{float: 'right', marginTop: 30, width: 120, marginLeft: 20, border: "2px solid #39b287", borderRadius: 20}}>
              <p style={{color: "#39b287"}}>Patch Notes</p>
            </div>
          ) : (
            <div className="dialogButton" style={{float: 'right', marginTop: 30, width: 120, marginLeft: 20}} onMouseDown={() => this.setState({page: 'patchNotes'})}>
              <p>Patch Notes</p>
            </div>
          )
        }
        {
          this.state.page === 'Collaborate' ? (
            <div className="dialogButton" style={{float: 'right', marginTop: 30, width: 120,  border: "2px solid #39b287", borderRadius: 20}}>
              <p style={{color: "#39b287"}}>Collaborate</p>
            </div>
          ) : (
            <div className="dialogButton" style={{float: 'right', marginTop: 30, width: 120}} onMouseDown={() => this.setState({page: 'Collaborate'})}>
              <p>Collaborate</p>
            </div>
          )
        }
        <div style={{clear: 'both'}}/>
        {
          this.state.page === 'Collaborate' ? (
            ...
            <p className="sectionHeader">Patch Notes</p>
          )
        }
        {
          this.state.page === 'Collaborate' ? (
            <div>
            <p className="logoSubtext" style={{lineHeight: "180%", fontSize: 18, marginTop: 40}}>Help Capsule improve by providing feedback and suggestions. Suggestions can range from new styling options to overall criticism of Capsule. Styling options can range from new card background colors, sticker sets, fonts, and more! Please check out the patch notes for changes made to Capsule.</p>
            {
